...
export class AFFNotReadyError extends Error {
  ...
  constructor() {
    super(
      'AFF not ready yet. Remember awaiting the "aff.ready" promise, before accessing any methods or properties.',
    );
  }
}
    ...
    this.ready = instantiateWA(
      input,
      {
        ...
        wasi_snapshot_preview1: {
           ...
           */
          environ_sizes_get: (
            penvironCountPtr: number,
            penvironBufferSizePtr: number,
          ): 0 | -1 => {
...
export default async function init(
  input: Request | RequestInfo | URL | Response | Promise<Response> =
    "/grad_aff_paa.wasm",
  options?: AFFOptions,
): Promise<AFF> {
  if (
